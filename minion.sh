...
MASTER_IP="10.10.103.86"
# You can use minion's ip instead
HOSTNAME="10.10.103.87"
USER="emma"
K8S_VERSION=1.0.0
REPO=dalanlan
...
install_docker() {
	...
	case "$lsb_dist" in
		...
		ubuntu|debian|linuxmint)
			...
			(
				...
				$sh_c 'sleep 3; apt-get update; apt-get install -y -q lxc-docker-1.7.1'
			)
			if command_exists docker && [ -e /var/run/docker.sock ]; then
				(
					set -x
	...
	sudo docker load -i ./tarpackage/p.tar #pause
...
install_k8s_minion() {
	...
	# Start minion
	sudo docker run --net=host -d --privileged -v /var/run/docker.sock:/var/run/docker.sock  ${REPO}/hyperkube:v${K8S_VERSION} /hyperkube kubelet --api-servers=https://${MASTER_IP}:6443 --v=2 --address=0.0.0.0 --enable-server --hostname-override=${HOSTNAME} --kubeconfig=/srv/kubernetes/config
	sudo docker run -d --net=host --privileged ${REPO}/hyperkube:v${K8S_VERSION} /hyperkube proxy --master=https://${MASTER_IP}:6443 --v=2 --kubeconfig=/srv/kubernetes/config
