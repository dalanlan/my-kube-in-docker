...
# determine whether we need to install registry for user
...
start_k8s(){
    ...
    flannelCID=$(sudo docker -H unix:///var/run/docker-bootstrap.sock run -d --net=host --privileged -v /dev/net:/dev/net quay.io/coreos/flannel:0.3.0 /opt/bin/flanneld -iface="eth0")
	...
	echo "Done set up ectd and flannel"
...
install_gorouter(){
	echo "Install gorouter"
	docker run --net=host --restart=on-failure:10 -d liuyilun/gorouter
}
