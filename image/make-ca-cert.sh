...
#mv ../easy-rsa.tar.gz tmpdir
tar xzf easy-rsa.tar.gz 

echo "Done tar package" 

cd easy-rsa-master/easyrsa3
./easyrsa init-pki 
./easyrsa --batch "--req-cn=$cert_ip@`date +%s`" build-ca nopass 
if [ $use_cn = "true" ]; then
    ./easyrsa build-server-full $cert_ip nopass 
    cp -p pki/issued/$cert_ip.crt "${cert_dir}/server.cert" 
    cp -p pki/private/$cert_ip.key "${cert_dir}/server.key"
else
...
echo "Done making server.key & server.crt"
...
echo "Done making cli.key & cli.crt"
